...
router.get("/getSchedulesDuringMonthAndYear", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      connection.query(
        "SELECT * FROM schedules WHERE MONTH(dateIn) = (?) AND YEAR(dateIn) = (?)",
        [month, year],
        (err, rows, fields) => {
          if (err) {
...
router.get("/getReservationsDuringMonthAndYear", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      // if there is no issue obtaining a connection, execute query
      connection.query(
        "SELECT * FROM reservations WHERE MONTH(dateIn) = (?) AND YEAR(dateIn) = (?)",
        [month, year],
        (err, rows, fields) => {
          if (err) {
...
router.post("/reservation", (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) {
      ...
      // if there is no issue obtaining a connection, execute query
      connection.query(
        "INSERT INTO reservations (roomId, dateIn, dateOut, userId, creatorType) values(?,?,?,?,?)",
        [roomId, dateIn, dateOut, userId, creatorType],
        (err, rows, fields) => {
          if (err) {
