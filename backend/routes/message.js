...
app.get('/api/EmployeeMessages', (req, res) => {
  ...
  pool.getConnection(async function (err, connection) {
    if (err) {
      ...
      let sql =
        'SELECT u.firstname, u.lastname, i.subject, i.message FROM inbox i LEFT JOIN users u ON i.senderId = u.userId WHERE i.recipientID = ?';
      let crypto;
      try {
        crypto = await import('crypto');
      } catch (err) {
...
app.get('/api/searchMessages', (req, res) => {
  ...
  pool.getConnection(async function (err, connection) {
    if (err) {
      ...
      let search = '%' + keyword + '%';
      let sql =
        'SELECT u.firstname, u.lastname, i.subject, i.message FROM inbox i LEFT JOIN users u ON i.senderId = u.userId WHERE i.message LIKE ?';
      let crypto;
      try {
        crypto = await import('crypto');
      } catch (err) {
...
app.post('/api/postMessage', (req, res) => {
  ...
  pool.getConnection(function (err, connection) {
    if (err) {
      ...
      let values = [[senderId, recipientID, subject, sendDate, message]];
